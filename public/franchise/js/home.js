...
userRequest.get(`${protocol}//${domain}/franchise/getOpenStatus`)
    .then((response) => {
    ...
    .then((openStatus) => {
        ...
        if (openLogs.length == 0) { status = 'unreported'; }
        else if (openLog.open_status == 0) { status = 'relax'; }
        else if (openLog.open_status == 1) { status = 'open' }
        else if (openLog.open_status == 2 && openLog.report_status == 0) { status = 'closed' }
        else if (openLog.open_status == 2 && openLog.report_status == 1) { status = 'reported' }
        else if (unreportedLogs.length != 0) { unreported = true; }
    ...
    .then((openStatus) => {
        ...
        switch (status) {
            ...
            case 'open': {
                ...
                loadStatus(open.id, open.report_date, status, open.open_time, open.open_location)
        ...
        if (unreportedLogs.length != 0) {
            for (let i = 0; i < unreportedLogs.length; i++) {
                ...
                loadStatus(log.id, log.report_date, 'late', log.open_time, log.open_location)
    ...
    .catch((err) => {
        ...
        if (error.status == 403 && error.data.msg == 'jwt expired') { loginAgain(); }
...
async function reportLocation(clicked_id) {
    ...
    switch (action) {
        case 'report-open': {
            Swal.fire({
                ...
                preConfirm: (location) => {
                    ...
                    return userRequest.post(`${protocol}//${domain}/franchise/updateOpenStatus`, openInfo)
                        .then((response) => {
        ...
        case 'report-relax': {
            openInfo.status = 0;
            await userRequest.post(`${protocol}//${domain}/franchise/updateOpenStatus`, openInfo)
                .then((response) => {
                    const reportResult = response.data.data
                    console.log(reportResult)
                    socket.emit('report-status', reportResult);
