...
const registerEnterprise = async (req, res) => {
	...
	const enterprise = await Enterprise.registerEnterprise(company, fullname, email);
	if (enterprise.error) {
	...
	const mailOptions = {
		...
		subject: '完成您的密碼設置',
		html: await sendMailContent(enterprise.fullname, domain, enterprise.id, enterprise.email)
	};
