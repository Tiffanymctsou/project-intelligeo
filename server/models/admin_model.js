...
const getSelectedLocation = async (city, town) => {
    const townInfo = await query(
        "SELECT town_code, coordinate FROM city_town WHERE city = ? AND town = ?",
        [city, town]
    );
    return { townInfo };
};
...
const getFranchise = async () => {
    const sqlQuery =
        "SELECT franchise_id, fullname, city, email, phone, address, join_date FROM franchise;";
