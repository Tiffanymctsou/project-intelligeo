...
const verifySetting = async (id, uid) => {
    const user = await query(
        "SELECT email, password FROM user WHERE account = ?",
        [id]
    );
...
const nativeLogin = async (account, password, expire) => {
    try {
        ...
        const accessToken =
            "Bearer " +
            jwt.sign({ account: account, password: password }, secret, {
                expiresIn: expire,
            });
...
const getUnreported = async (franchise_id) => {
    ...
    const unreportedLog = await query(sqlQuery, [
        franchise_id,
        "-",
        today,
        2,
...
const updateOpenStatus = async (franchise_id, location, status) => {
    try {
        ...
        const franchise = await query(
            "SELECT fullname FROM franchise WHERE franchise_id = ?",
            [openLog.franchise_id]
        );
...
const reportClose = async (status, log_id) => {
    try {
        ...
        const time = moment.tz("Asia/Taipei").format("H:mm:ss");
        const sqlQuery =
            "UPDATE open_log SET close_time = ?, open_status = ? WHERE id = ?";
...
const reportSales = async (log_id, amount) => {
    try {
        ...
        const time = moment.tz("Asia/Taipei").format("H:mm:ss");
        const updateSql =
            "UPDATE open_log SET report_status = ?, report_time = ? WHERE id = ?";
